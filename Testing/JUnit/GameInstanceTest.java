...
import functional.testingcomponents.*;
...
class GameInstanceTest
{
    ...
    private final String GRID_SAVE_PATH = System.getProperty("user.dir") + "/Testing/TestAssets/TestGridSave.ser";
    ...
    void setUp()
    {
        ...
        try
        {
            img1 = ImageIO.read(new File(System.getProperty("user.dir") + "/Testing/TestAssets/img1.png"));
            img2 = ImageIO.read(new File(System.getProperty("user.dir") + "/Testing/TestAssets/img2.png"));
            img3 = ImageIO.read(new File(System.getProperty("user.dir") + "/Testing/TestAssets/img3.png"));
    ...
    void assertTest()
    {
        //assertThrows(AssertionError.class,()->1+1)
    ...
    void loadTextureAssets()
    {
        try
        {
            game1.loadTextureAssets(System.getProperty("user.dir") + "/Testing/TestAssets");
    ...
    void removeAllMappedUserInputFromFrame()
    {
        try
        {
            GameObject[] gameObjects = new GameObject[Math.min(game1.getMainGrid().getGridWidth(), game1.getMainGrid().getGridHeight())];
            int i = 0;
            for (i = 0; i < Math.min(game1.getMainGrid().getGridWidth(), game1.getMainGrid().getGridHeight()); i++)
            {
                gameObjects[i] = getTestObject(game1.getMainGrid(), false, i, i);
                gameObjects[i].addComponent(new TestControllableComponent(gameObjects[i], game1.getUserInputManager(), i));
                game1.addGameObject(gameObjects[i], i, i);
            ...
            //removeAllMappedUserInputFromFrame(); is called within the mainGrid.removeGameObject()
            assertEquals(i, game1.getUserInputManager().getKeyboardInputs().size());
            for (GameObject g : game1.getGameObjects(TestControllableComponent.class))
            {
                mainGrid.removeGameObject(g.getX(), g.getY());
            }
            assertEquals(0, game1.getUserInputManager().getKeyboardInputs().size());
            assertEquals(0, game1.getGameObjects(TestControllableComponent.class).length);
    ...
    void addEvent()
    {
        try
        {
            ...
            game1.addEvent(testEvent);
            game1.startPeriodicEventChecking();
            game1.setUpdateTimer(5);
            game1.start();
            Thread.sleep(10);
            synchronized (game1)
            {
            ...
            Thread.sleep(100);
            assertTrue(testEvent.hasRunEvent);
    ...
    void checkEvents()
    {
        try
        {
            ...
            game1.start();
            Thread.sleep(100);
            assertNotEquals(0, testEvent.numTimesTriggerChecked);
        } catch (Exception e)
        {
            e.printStackTrace();
            fail();
    ...
    void startPeriodicEventChecking()
    {
        try
        {
            ...
            TestEvent[] testEvents = new TestEvent[100];
            for (int i = 0; i < 100; i++)
            {
                TestEvent testEvent = new TestEvent();
                game1.addEvent(testEvent);
    ...
    private class TestEvent implements EventTrigger
    {
        public boolean trigger = false;
        public boolean hasRunEvent = false;
        public int numTimesEventRun = 0;
        public int numTimesTriggerChecked = 0;

        @Override
        public boolean checkTrigger()
        {
            numTimesTriggerChecked++;
            return trigger;
        }
